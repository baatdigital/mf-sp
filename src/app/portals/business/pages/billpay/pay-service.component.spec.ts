...
describe('PayServiceComponent', () => {
  ...
  // ============================================================================
  // FIX F-007: _submitLock + UUID v4 para idempotencia
  // ============================================================================

  it('F-007: confirmarPago debe generar UUID v4 como idempotency_key al consultar (no Date.now)', async () => {
    ...
    // Verificar que el idempotency key es un UUID v4 válido
    const idemKey = (component as any)._idempotencyKey;
    expect(uuidRegex.test(idemKey)).toBeTrue();
    expect(idemKey).not.toContain('Date');
  });

  it('F-007: confirmarPago debe bloquear doble-click con _submitLock', async () => {
    // Usar Subject en lugar de of() para que el lock NO se libere antes del segundo click
    const pending$ = new Subject();
    mockBillpay.payBill.and.returnValue(pending$.asObservable());
    ...
    // Primer llamado: lock se setea a true, HTTP call inicia pero no completa
    component.confirmarPago();
    // Segundo llamado inmediato: debe ser bloqueado por _submitLock
    component.confirmarPago();

    // Solo un HTTP call debio realizarse
    expect(mockBillpay.payBill).toHaveBeenCalledTimes(1);

    // Limpiar Subject
    pending$.complete();
  });

  it('F-007: confirmarPago debe liberar _submitLock tras éxito', async () => {
    component.queryResult.set(mockQueryResult);
    component.accounts.set(mockAccounts);
    component.selectedAccountId = 'ACC-001';
    component.currentStep.set(2);
    ...
    expect((component as any)._submitLock).toBeFalse();
    expect(mockBillpay.payBill).toHaveBeenCalled();
  });

  it('F-007: confirmarPago debe liberar _submitLock tras error y regenerar idempotency key', async () => {
    mockBillpay.payBill.and.returnValue(throwError(() => new Error('Payment error')));
    component.queryResult.set(mockQueryResult);
    component.accounts.set(mockAccounts);
    component.selectedAccountId = 'ACC-001';
    ...
    const oldKey = (component as any)._idempotencyKey;
    component.confirmarPago();
    await fixture.whenStable();

    expect((component as any)._submitLock).toBeFalse();
    // Key debe ser diferente tras error (para permitir reintento)
    expect((component as any)._idempotencyKey).not.toBe(oldKey);
  });

  it('F-007: confirmarPago debe enviar idempotency_key en el payload', async () => {
    component.queryResult.set(mockQueryResult);
    component.accounts.set(mockAccounts);
    component.selectedAccountId = 'ACC-001';
    component.currentStep.set(2);
    const expectedKey = (component as any)._idempotencyKey;

    component.confirmarPago();
    await fixture.whenStable();

    const payCall = mockBillpay.payBill.calls.first();
    const payload = payCall.args[1];
    expect(payload.idempotency_key).toBe(expectedKey);
  });
});
